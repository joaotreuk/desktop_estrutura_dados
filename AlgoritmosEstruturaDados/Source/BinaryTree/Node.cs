...
namespace BinaryTree
{
    ...
    public class Node
    {
        ...
        /// </summary>
        private static readonly Bitmap _nodeBg = new Bitmap(30, 25);
        /// <summary>
        ...
        static Node()
        {
            Graphics g = Graphics.FromImage(_nodeBg);                                    // get a Graphics from _nodeBg bitmap, 
            g.SmoothingMode = SmoothingMode.HighQuality;                            // set the smoothing mode
            Rectangle rcl = new Rectangle(1, 1, _nodeBg.Width - 2, _nodeBg.Height - 2);   // get a rectangle of drawer
        ...
        public bool Remove(int nodeValue, out bool containedOnMe)
        {
            ...
            bool result = nodeToRemove.Remove(nodeValue, out bool containOnChild);
            if (containOnChild) // if the child of the current node contained the nodeValue, that child should be removed.
            {
                ...
                else                                                        // left and right are not null
                {
                    ...
                    else                                                    // 4. the removing nodes' right child has left child
                    {
                        ...
                        for (Node node = nodeToRemove.Right; node != null; node = node.Left)
                        {
                            if (node.Left == null)
                            {
        ...
        private static readonly Font font = new Font("Tahoma", 14f * Coef);
        /// <summary>
        ...
        public Image Draw(out int center)
        {
            ...
            Size lSize = new Size();
            Size rSize = new Size();
            bool under = (lNodeImg != null) || (rNodeImg != null);// if true the current node has childs
            if (lNodeImg != null)
            {
                lSize = lNodeImg.Size;
            }
            ...
            Pen pen = new Pen(Brushes.Black, 1.2f * Coef)
            {
